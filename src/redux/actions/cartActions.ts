...
export type StartCartActAction = {
...
export type CartActSuccessAction = {
    type: typeof ACTIONS.CART_ACT_SUCCESS;
    payload: {
        message: string;
        cart: Variant[]; // Adjust the type of cart items as needed
    };
}

export type CartActFailAction = {
...
export type Actions = StartCartActAction | FetchCartAction | CartActSuccessAction | CartActFailAction;

export const addToCart = (data: AddToCartItem) => async (dispatch: Dispatch<Actions>) => {
    dispatch({ type: ACTIONS.START_CART_ACT })
    ...
    try {
        ...
        if (res.data.status === 'success') {
            dispatch({
                ...
                payload: { cart: res.data.data.cart },
            })
        }
    } catch (err: any) {
        console.error(err)
...
export const updateCart = (data: UpdateInCartItem) => async (dispatch: Dispatch<Actions>) => {
    ...
    try {
        ...
        if (res.data.status === 'success') {
            dispatch({
                type: ACTIONS.CART_ACT_SUCCESS,
                payload: { message: 'Cart update successfully', cart: res.data.data.cart },
            })
        }
    } catch (err: any) {
        console.error(err)
...
export const resetMessageError = () => {
    return { type: ACTIONS.RESET_MESSAGE_ERROR }
}
